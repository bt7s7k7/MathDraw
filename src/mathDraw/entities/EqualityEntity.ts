    ...
    export function parse(code: string, variable: string, rootExpression: Expression): Entity {
        ...
        const parseExpression = (expression: Expression) => {
            if (expression.type == "BinaryExpression") {
                if (expression.operator == "*" && (expression.left.type == "Literal" || expression.left.type == "Identifier") && (expression.right.type == "Identifier" || expression.right.type == "BinaryExpression" && (expression.right.operator == "**" || expression.right.operator == "*"))) {
